...
namespace DeadCellsBossFight.Items
{
    public class Homo3 : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (BottleSystem.AddBottle == 0)
            {
                BottleSystem.AddBottle = 2;

            }
            else if (BottleSystem.AddBottle == 1)
            {
                BottleSystem.AddBottle = 3;

            }
            else if (BottleSystem.AddBottle == 4)
            {
                BottleSystem.AddBottle = -1;
